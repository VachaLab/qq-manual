# Controlling when transfer from the working directory is performed
...
You can specify multiple transfer modes for [`qq submit`](commands/qq_submit.md) by separating them with commas, colons, or spaces. If the condition for **any** of the specified transfer modes is satisfied, the files are transferred and the working directory is removed.
...
Transfer mode does not affect how files are archived in [loop jobs](job_types/loop_job.md). For example, if you run a loop job with `--transfer-mode always` and the job fails, **archiving is not performed** and **all files from the working directory are transferred to the input directory**.
