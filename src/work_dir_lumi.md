...
> - **IMPORTANT!** On LUMI, you are billed for the amount of storage you use! By default, qq only clears the working directory if the job is successfully completed. If your job fails, you MUST clear the working directory yourself (see [`qq wipe`](qq_wipe.md)), otherwise you will get billed for it! Alternatively, you can select the [`always` transfer mode](transfer_modes.md#transfer-modes) when submitting the job.
